...
Ext.define('Extensible', {
    ...
    */
    Date: {
         ...
         *
         * TODO This function does not return correct results on days where the daylight savings time starts in time
         * zones that switch at midnight, for example Brasilia Time (http://www.timeanddate.com/time/change/brazil/brasilia)
         * and Australia Eastern Standard Time (http://www.timeanddate.com/time/change/australia/sydney).
         *
        ...
        * If a value is passed for optional parameter ndays, then the returned date will be set to ndays from the
        * passed date.
        *
        * Use this function whenever you need to determine the beginning of a day or need to increment / decrement
        * dates. Using this function ensures that the calculation is performed in a way that is taking daylight savings
        * switches properly into account.
        *
        * Examples
        * <pre><code>
// Find beginning of the current day:
dt = Extensible.getDayBeginning(currentDt);

// Find beginning of the next day:
dt = Extensible.getDayBeginning(currentDt, 1);

// Find beginning of the day seven days from the passed date:
dt = Extensible.getDayBeginning(currentDt, 7);
        * </pre></code>
        *
        * Note that while most days start at 12:00am there are some days that start at 01:00am. These are the days
        * when daylight savings time starts in time zone where daylight savings time switches at midnight.
        *
        * @param {Date} dt Date objects for which the day beginning is calculated.
        * @param {int} ndays (optional) Number of days to increment the passed Date object. Supported value are
        * positive integers, zero and negative integers.
        * @return {Date} A new Date object with time set to the beginning of the day and the date set to ndays days
        ...
        */
        getDayBeginning: function(dt, ndays) {
           ...
           var D = Ext.Date;
           ndays = (ndays === undefined) ? 0 : ndays;
           var dt2 =  D.add(dt, D.HOUR, 24 * ndays + 12 - dt.getHours());
           var dt3 = this.clearTime(dt2, false);
           return dt3;
       },
        ...
        * passed date.
        *
        * Use this function whenever you need to determine the end of a day and need to increment / decrement
        * dates. Using this function ensures that the calculation is performed in a way that is taking daylight savings
        * switches properly into account.
        *
        * Examples
        * <pre><code>
// Find end of the current day:
dt = Extensible.getDayEnd(currentDt);

// Find end of the next day:
dt = Extensible.getDayEnd(currentDt, 1);

// Find end of the day seven days from the passed date:
dt = Extensible.getDayEnd(currentDt, 7);
        * </pre></code>
        *        *
        * @param {Date} dt Date objects for which the day end is calculated.
        * @param {int} ndays (optional) Number of days to increment the passed Date object. Supported value are
        * positive integers, zero and negative integers. Default value is 0.
        * @param {string} res (optional) Supported values are "minutes", "seconds" or "millis". Default value is
        * "millis". This parameter controls how the end of a day is calculated. This function subtracts one unit of time
        * as specific by this parameter from midnight to calculate the end time of a day. For example, if this
        * parameter is set to "minutes" then the end time of a day is 23:59:00. If this parameter is set to second,
        * then the end time of a day is 23:59:59. And, if this parameter is set to "millis" then the end time of a
        * day is set to 23:59:59.999.
        * @return {Date} A new Date object with time set to the end of the day and the date set to ndays days
        * from the passed date.
        */
       getDayEnd: function(dt, ndays, res) {
            var unit,
                D = Ext.Date;
            ndays = (ndays === undefined) ? 0 : ndays;
            if (res === 'minutes') {
